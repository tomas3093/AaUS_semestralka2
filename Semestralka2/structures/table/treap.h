...
namespace structures
{
	...
	inline void Treap<K, T>::insert(const K & key, const T & data)
	{
		...
		typename BinarySearchTree<K, T>::BSTTreeNode * treeNode = new typename BinarySearchTree<K, T>::BSTTreeNode(treapItem);

		if (this->tryToInsertNode(treeNode))
		{
			...
			while (parentPriority != INT_MIN && parentPriority > treapItem->getPriority())
			{
				...
				if (resetRoot)
				{
					this->binaryTree_->replaceRoot(treeNode);
				}
	...
	inline T Treap<K, T>::remove(const K & key)
	{
		...
		typename BinarySearchTree<K, T>::BSTTreeNode* nodeToRemove = this->findBSTNode(key, found);
