...
namespace structures
{
	...
	void Heap<T>::push(const int priority, const T& data)
	{
		...
		int currentIndex = static_cast<int>(this->list_->size()) - 1;
		int parentIndex = getParentIndex(currentIndex);
		while (parentIndex >= 0 && (*this->list_)[currentIndex]->getPriority() < (*this->list_)[parentIndex]->getPriority())
		{
	...
	T Heap<T>::pop()
	{
		...
		(*this->list_)[0] = (*this->list_)[static_cast<int>(this->list_->size()) - 1];
		this->list_->removeAt(static_cast<int>(this->list_->size()) - 1);

		int currentIndex = 0;
		int sonIndex = getGreaterSonIndex(currentIndex);
		while (sonIndex < static_cast<int>(this->list_->size()) && (*this->list_)[currentIndex]->getPriority() > (*this->list_)[sonIndex]->getPriority())
		{
			DSRoutines::swap((*this->list_)[currentIndex], (*this->list_)[sonIndex]);
	...
	{
		int size = static_cast<int>(this->list_->size());
		int lSonIndex = 2 * index + 1;
		int rSonIndex = 2 * index + 2;
		PriorityQueueItem<T>* lSon = lSonIndex < size ? (*this->list_)[lSonIndex] : nullptr;
		PriorityQueueItem<T>* rSon = rSonIndex < size ? (*this->list_)[rSonIndex] : nullptr;
